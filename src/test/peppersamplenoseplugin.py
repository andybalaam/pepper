...
class RunPepperTest( unittest.TestCase ):
    ...
    def runProgram( self ):
        """Check that running the supplied program with pepper prints the
        expected results.
        """
    ...
    def generateFile( self ):
        """Check that running pepper on input file returns output file.
        """
    ...
    def extractExpected( self, expected_stdout ):
        expected_retval = 0
        m = RunPepperTest.retval_re.match( expected_stdout )
        if m:
    ...
    def extractStdErrLines( self, expected_stdout ):
        ...
        for ln in expected_stdout.split( '\n' ):
            m = RunPepperTest.stderr_re.match( ln )
            if m:
...
class PepperSampleNosePlugin( Plugin ):
    """
    Find pepper code samples and run them as tests.
    If a directory contains 2 or more files with extensions from this list:
      ...
      .cpp
      .output
    with matching names before the extensionm the pepper executable will be
    ...
    pepper on the first file in found.
    """
        ...
        for name, files in name2files.iteritems():
            ...
            for ext, filename in files:
                if prev_name is not None:
                    yield RunPepperTest( path, prev_name, filename )
                prev_name = filename
