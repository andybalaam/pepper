...
class Executor( object ):
    def __init__( self, sys_op ):
        self.build_steps = [
            SourceBuildStep(),   # PepperOptions.SOURCE = 0
            LexBuildStep(),      # PepperOptions.LEXED  = 1
            ParseBuildStep(),    # PepperOptions.PARSED = 2
            RenderBuildStep(),   # PepperOptions.CPP    = 3
            ]
...
def _temp_dir_file( sys_op, filename ):
    ...
    # TODO: make the dir near the .pepper file if possible, or in /tmp
    # otherwise?
...
def process_options( opts, sys_op, executor ):
    ...
    with sys_op.open_read( inf.filename ) as in_fl:
        ...
        for i in range( inf.filetype + 1,
                min( PepperOptions.EXE, ouf.filetype + 1 ) ):
        ...
        if ouf.filetype == PepperOptions.EXE:
            executor.cppcompiler.run( val, ouf.filename )
        elif ouf.filetype == PepperOptions.RUN:
...
def main( argv, stderr ):
    ...
    return parse_and_process_options( argv, PepperOptions, SystemOperations,
        Executor, stderr )
